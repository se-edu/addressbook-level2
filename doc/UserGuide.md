...
and persons matching at least one keyword will be returned (i.e. `OR` search). The tags can also e searched for,
which is also NOT case-sensitive.

Examples:
* `find John`<br>
  Returns `John Doe` but not `john`
* `find Betsy Tim John`<br>
  Returns Any person having names `Betsy`, 'betsy', 'BETSY' `Tim`, 'tim', 'Tim', 'TIM' or `John`, 'John', or 'JOHN'
* `find Amy friend aunt`<br>
  Returns Any person having names `Amy`, `AMY`, 'amy' or having tags 'friend', 'FRIEND', 'Friend', `aunt`,
  'AUNT', or 'Aunt'
